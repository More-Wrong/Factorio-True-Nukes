...
script.on_init(function()
	...
	global.optimisedNukes = {} 				-- has keys: 
	    ...
	    --used for doing chunk-by-chunk loading of detonation results... pretty weird, but should work fine, needs to be careful of rng chunk order safety (so needs to seed its rng off of loaded chunk, not use global one)
			...
			if(global.cratersFastData[surface].xDone[x]==nil) then	--ignore all the ones we have already done
				...
				for y,foundChunkH in pairs(xchunks) do
					...
					for _,t in pairs(targetTiles) do
						...
						local h3;
						local h4;
						if(game.surfaces[surface].is_chunk_generated({t.position.x/32, (t.position.y+1)/32})) then
							h1 = waterInCraterGoingOutDepths[game.surfaces[surface].get_tile(t.position.x, t.position.y+1).name];
						else
							h1 = currentH
						end
						if(game.surfaces[surface].is_chunk_generated({t.position.x/32, (t.position.y-1)/32})) then
							h2 = waterInCraterGoingOutDepths[game.surfaces[surface].get_tile(t.position.x, t.position.y-1).name];
						else
							h2 = currentH
						end
						if(game.surfaces[surface].is_chunk_generated({(t.position.x+1)/32, t.position.y/32})) then
							h3 = waterInCraterGoingOutDepths[game.surfaces[surface].get_tile(t.position.x+1, t.position.y).name];
						else
							h3 = currentH
						end
						if(game.surfaces[surface].is_chunk_generated({(t.position.x-1)/32, t.position.y/32})) then
							h4 = waterInCraterGoingOutDepths[game.surfaces[surface].get_tile(t.position.x-1, t.position.y).name];
						else
							h4 = currentH
						end
...
local function optimisedChunkLoadHandler(chunkPosAndArea, chunkLoaderStruct, killPlanes)
	...
	if  ((minR<chunkLoaderStruct.blast_max_r) or (minR<chunkLoaderStruct.thermal_max_r)) then
		...
		local new_rng_seed = (chunkPosAndArea.x * 374761393 + chunkPosAndArea.y * 668265263 + map_seed) % 4294967296
		local rng = game.create_random_generator()
		rng.re_seed(new_rng_seed); -- bit expensive to do for every chunk, but not much choice really
		...
		if(minR<chunkLoaderStruct.crater_external_r*1.1+4) then
			...
			if(minR<crater_internal_r*1.1/3+10 and maxR> crater_internal_r/3-10) then
				tileNoiseLimited(game.surfaces[surface_index], tileTable, originPos, crater_internal_r/3, 1, {default = depthsForCrater[-3]}, 3, startAngle, endAngle, chunkPosAndArea.area);
			end
			if(minR<crater_internal_r*1.1*2/3+10 and maxR> crater_internal_r*2/3-10) then
				tileNoiseLimited(game.surfaces[surface_index], tileTable, originPos, crater_internal_r*2/3, 1, {default = depthsForCrater[-2]}, 3, startAngle, endAngle, chunkPosAndArea.area);
			end
			if(minR<crater_internal_r*1.15+10 and maxR> crater_internal_r-10) then
				tileNoiseLimited(game.surfaces[surface_index], tileTable, originPos, crater_internal_r, 2, {default = depthsForCrater[0]}, 3, startAngle, endAngle, chunkPosAndArea.area);
				tileNoiseLimited(game.surfaces[surface_index], tileTable, originPos, crater_internal_r, 1, {default = depthsForCrater[-1]}, 3, startAngle, endAngle, chunkPosAndArea.area);
			end
			game.surfaces[surface_index].set_tiles(tileTable)
			tileTable={};
			-- ensure noise for crater goes on top of lakes
			...
			if(minR<crater_external_r*1.1+10 and maxR> crater_external_r-10) then
				tileNoiseLimited(game.surfaces[surface_index], tileTable, originPos, crater_external_r, 1, {default = "nuclear-ground"}, 3, startAngle, endAngle, chunkPosAndArea.area);
			end
			--high noise around crater
			if(minR<crater_external_r*1.1+6 and maxR> crater_external_r-14) then
				tileNoiseLimited(game.surfaces[surface_index], tileTable, originPos, crater_external_r-4, 1, {default = "nuclear-high"}, 3, startAngle, endAngle, chunkPosAndArea.area);
			end
